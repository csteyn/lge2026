...
**Known bias.** Because the 2019 shares are themselves noisy, `b` is biased towards zero (attenuation). On synthetic data with known truth the slope for one party was recovered as 0.78 against a true 0.90 (check D01). This makes the baseline slightly too uniform across VDs. A measurement-error correction is planned.
